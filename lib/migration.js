...
module.exports = function(DB2) {
  ...
  DB2.prototype.getTableStatus = function(model, cb) {
    ...
    columnSQL = 'SELECT column_name AS NAME, type_name AS DATATYPE, ' +
      'ordinal_position AS COLNO, column_size AS DATALENGTH, ' +
      'nullable AS NULLS ' +
    ...
    self.execute(columnSQL, function(err, tableInfo) {
      if (err) {
        ...
        indexSQL = 'SELECT table_name, table_schema, index_name, ' +
  ...
  DB2.prototype.alterTable = function(model, actualFields, actualIndexes,
                                      callback, checkOnly) {
    debug('DB2.prototype.alterTable %j %j %j %j',
